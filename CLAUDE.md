...
3. **AST Building** (`Ast/AstBuilder.cs`) — S-expressions → `AstNode.Program` (handles special forms: `define`, `let`, `if`, `lambda`, `match`, `record`, `struct`, `union`, `class`, `interface`, `provide`, `with`, `object`, etc.)
...
Between stages 5 and 6, **continuation lowering** runs when the program uses `call/cc`, `shift`/`reset`, `control`/`prompt` or `call/comp`: `TailCallAnalyzer` (which only sets `IrNode.IsTailCall` for this pipeline — TCO itself is `TailCallLowering`'s job), then `AsyncContinuationAnalyzer`, `CrossAssemblyCallCcAnalyzer`, `CapturableCallHoister` and `ContinuationTransform`. The frames it synthesizes bottom out in `src/ZScheme.Runtime/`; see `docs/CONTINUATIONS.md`.
