...
6. **Code Generation** (`Codegen/CSharpEmitter.cs` or `Codegen/CecilEmitter.cs`) — IR → C# source or IL (via Mono.Cecil). TCO is lowered to `while(true)` loops in C#. CLR interop handled by `ClrInterop.cs`.
