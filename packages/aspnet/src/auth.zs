...
(import-clr
  System
  System.Text
  [from-base64 Convert/FromBase64String : (String -> (Mutable-Vector Byte))]
  [utf8-get-string UTF8Encoding.GetString
    :instance : (UTF8Encoding (Mutable-Vector Byte) -> String)]
  [str-index-of String.IndexOf :instance : (String String -> Int)]
  [str-substring-from String.Substring :instance : (String Int -> String)]
  [str-substring-range String.Substring :instance : (String Int Int -> String)])
...
(define (auth/check-basic [auth-header : String] [user : String] [pass : String]) : Bool
  (if (starts-with? auth-header "Basic ")
      ...
      (with-handlers ([FormatException __e] #f)
