...
(test-suite ResultTests
  ...
  (test-case map_transforms_ok
    (check-equal? (Ok 10) (map (Ok 5) (lambda (x) (* x 2)))))

  (test-case map_preserves_err
    (check-true (err? (map (Err "fail") (lambda (x) (* x 2))))))

  (test-case flat_map_chains_ok
    (check-equal? (Ok 10)
      (flat-map (Ok 5) (lambda (x) (Ok (* x 2))))))

  (test-case flat_map_returns_err
    (check-true (err? (flat-map (Err "fail") (lambda (x) (Ok x)))))))
