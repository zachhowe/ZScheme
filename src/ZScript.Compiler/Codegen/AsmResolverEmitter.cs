...
/// </summary>
public sealed class AsmResolverEmitter(
    string assemblyName,
    ...
    bool isModule = false)
{
    ...
    {
        Log.Debug("AsmResolverEmitter: emitting assembly {AssemblyName}", assemblyName);
        ...
        Log.Debug("AsmResolverEmitter: emit complete, {ByteCount} bytes", bytes.Length);
        return bytes;
    }

    private void LoadPrecompiledAssembly(string path)
    {
        Log.Debug("AsmResolverEmitter: loading precompiled assembly {Path}", path);
        Assembly asm;
        try
        {
    ...
    {
        Log.Debug("AsmResolverEmitter: defining interface type {InterfaceName}", iface.Name);
    ...
    {
        Log.Debug("AsmResolverEmitter: defining record type {RecordName}", record.Name);
        var ns = parentType is null ? _ilNamespace : "";
        var vis = parentType is null ? TypeAttributes.Public : TypeAttributes.NestedPublic;
    ...
    {
        Log.Debug("AsmResolverEmitter: defining union type {UnionName}", union.Name);
        var ns = parentType is null ? _ilNamespace : "";
        var vis = parentType is null ? TypeAttributes.Public : TypeAttributes.NestedPublic;
    ...
    {
        Log.Debug("AsmResolverEmitter: emitting function {FuncName}, IsAsync={IsAsync}, IsGeneric={IsGeneric}",
    ...
    {
        Log.Debug("AsmResolverEmitter: emitting async state machine for {FuncName}", func.Name);
