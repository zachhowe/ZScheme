...
public sealed class StdlibConcurrentCollectionGenerator
{
    ...
    public bool QueueImported() => _ctx.Imports.Contains(StdlibImport.ConcurrentQueue);
    public bool StackImported() => _ctx.Imports.Contains(StdlibImport.ConcurrentStack);
    public bool BagImported() => _ctx.Imports.Contains(StdlibImport.ConcurrentBag);
    public bool DictionaryImported() => _ctx.Imports.Contains(StdlibImport.ConcurrentDictionary);
    ...
    public string QueueCountToInt(Scope scope, int depth) =>
        BuildAndReduce(scope, depth, "concurrent-queue/new", "concurrent-queue/enqueue!", "concurrent-queue/count");
    ...
    public string QueueTryDequeueToInt(Scope scope, int depth) =>
        BuildAndReduceTryRead(scope, depth, "concurrent-queue/new", "concurrent-queue/enqueue!", "concurrent-queue/try-dequeue!");

    public string StackCountToInt(Scope scope, int depth) =>
        BuildAndReduce(scope, depth, "concurrent-stack/new", "concurrent-stack/push!", "concurrent-stack/count");

    public string StackTryPopToInt(Scope scope, int depth) =>
        BuildAndReduceTryRead(scope, depth, "concurrent-stack/new", "concurrent-stack/push!", "concurrent-stack/try-pop!");

    public string BagCountToInt(Scope scope, int depth) =>
        BuildAndReduce(scope, depth, "concurrent-bag/new", "concurrent-bag/add!", "concurrent-bag/count");

    public string BagTryTakeToInt(Scope scope, int depth) =>
        BuildAndReduceTryRead(scope, depth, "concurrent-bag/new", "concurrent-bag/add!", "concurrent-bag/try-take!");
    ...
    public string DictionaryCountToInt(Scope scope, int depth)
    {
        ...
        return $"(let [{v} (concurrent-dictionary/new)] (begin {string.Join(" ", puts)} (concurrent-dictionary/count {v})))";
    }
    ...
    public string DictionaryTryRemoveToInt(Scope scope, int depth)
    {
        ...
        return $"(let [{v} (concurrent-dictionary/new)] (begin (concurrent-dictionary/put! {v} 0 {seedVal}) (value/1 (concurrent-dictionary/try-remove! {v} 0))))";
    }
