...
public sealed partial class IlEmitter
{
    ...
    private void DefineInterfaceType(IrNode.InterfaceDecl iface, TypeDefinition? parentType = null)
    {
        Log.Debug(
            ...
            iface.Name,
            iface.Methods.Count,
            iface.TypeParams.Count,
            iface.BaseInterfaceNames.Count
        );
        ...
        var typeDef = new TypeDefinition(
            ns,
            Emitted(iface.EmitName, iface.Name),
        ...
        foreach (var method in iface.Methods)
        {
            var retType =
                method.ReturnType == ZType.Unit
                    ...
                    : MapToClr(method.ReturnType, EmitContext.Empty);
            var paramTypes = method.Params.Select(p => MapToClr(p.Type, EmitContext.Empty)).ToArray();
            ...
            for (var pi = 0; pi < method.Params.Count; pi++)
                methodDef.ParameterDefinitions.Add(
                    new ParameterDefinition((ushort)(pi + 1), Sanitize(method.Params[pi].Name), 0)
                );
    ...
    private void DefineRecordType(IrNode.RecordDecl record, TypeDefinition? parentType = null)
    {
        ...
        Log.Debug(
            "IlEmitter: defining record type {RecordName}, {FieldCount} fields, {TypeParamCount} type params",
            record.Name,
            record.Fields.Count,
            record.TypeParams.Count
        );
        var ns = parentType is null ? _ilNamespace : "";
        var vis = parentType is null ? TypeAttributes.Public : TypeAttributes.NestedPublic;
        var typeDef = new TypeDefinition(
            ns,
            record.EmitName ?? record.Name,
        ...
        if (record.TypeParams.Count > 0)
        {
            ...
            {
                var gp = new GenericParameterSignature(
                    _module,
        ...
        {
            var fieldClrType = MapToClr(field.Type, EmitContext.Empty, typeParamMap: typeParamMap);
        ...
        // Constructor
        var ctorParams = record.Fields.Select(f => MapToClr(f.Type, EmitContext.Empty, typeParamMap: typeParamMap)).ToArray();
        ...
        for (var i = 0; i < record.Fields.Count; i++)
            ctor.ParameterDefinitions.Add(
                new ParameterDefinition((ushort)(i + 1), Sanitize(record.Fields[i].Name), 0)
            );
    ...
    {
        Log.Debug(
            "IlEmitter: defining struct type {RecordName}, {FieldCount} fields, {TypeParamCount} type params",
            record.Name,
            record.Fields.Count,
            record.TypeParams.Count
        );
    ...
    /// </summary>
    private void RegisterSingleCasePattern(
        IrNode.RecordDecl record,
    ...
    /// </summary>
    private void EmitStructEquality(
        TypeDefinition typeDef,
        IReadOnlyList<FieldDefinition> backingFields
    )
    {
        ...
        IFieldDescriptor ResolveField(FieldDefinition f)
        {
            return closedSig is null
                ? f
                : new MemberReference(closedSig.ToTypeDefOrRef(), f.Name!, f.Signature!);
        }

        // --- Equals(T other) — structural equality, no null handling ---
        var equalsT = new MethodDefinition(
            "Equals",
        ...
        var equalsObj = new MethodDefinition(
            "Equals",
        ...
        // --- GetHashCode ---
        var getHash = new MethodDefinition(
            "GetHashCode",
            MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig,
        ...
        var opNeq = new MethodDefinition(
            "op_Inequality",
            MethodAttributes.Public
                | MethodAttributes.Static
                | MethodAttributes.SpecialName
    ...
    private void EmitEqualityContract(TypeDefinition typeDef)
    {
        ...
        var prop = new PropertyDefinition(
            "EqualityContract",
            0,
            PropertySignature.CreateInstance(typeSig)
        );
        prop.Semantics.Add(new MethodSemantics(getter, MethodSemanticsAttributes.Getter));
        typeDef.Properties.Add(prop);
    }
    ...
    )
    {
        ...
        var typeType = _module.DefaultImporter.ImportType(typeof(Type));
        var eqContractGetter = typeDef
            .Properties.FirstOrDefault(p => p.Name == "EqualityContract")
            ?.Semantics.FirstOrDefault(s => s.Attributes == MethodSemanticsAttributes.Getter)
            ?.Method;
        ...
        if (eqContractGetter is not null)
        {
            ...
            if (closedSig is not null)
                getterRef = new MemberReference(
                    closedSig.ToTypeDefOrRef(),
                    eqContractGetter.Name!,
                    eqContractGetter.Signature!
                );
    ...
    {
        var open = _module.DefaultImporter.ImportType(typeof(EqualityComparer<>));
    ...
    )
    {
        ...
        var copyCtor = new MethodDefinition(
            ".ctor",
            MethodAttributes.Family
                | MethodAttributes.HideBySig
                | MethodAttributes.SpecialName
    ...
    private void EmitPrintMembers(TypeDefinition typeDef)
    {
        ...
        var printMembers = new MethodDefinition(
            "PrintMembers",
    ...
    private void EmitCloneMethod(TypeDefinition typeDef, MethodDefinition copyCtor)
    {
        ...
        var cloneMethod = new MethodDefinition(
            "<Clone>$",
            MethodAttributes.Public | MethodAttributes.HideBySig,
    ...
    {
        Log.Debug(
            "IlEmitter: defining union type {UnionName}, {CaseCount} cases, {TypeParamCount} type params",
            union.Name,
            union.Cases.Count,
            union.TypeParams.Count
        );
        var ns = parentType is null ? _ilNamespace : "";
        var vis = parentType is null ? TypeAttributes.Public : TypeAttributes.NestedPublic;
        var baseType = new TypeDefinition(
            ns,
            union.EmitName ?? union.Name,
        ...
        foreach (var @case in union.Cases)
        {
            ...
            var caseType = new TypeDefinition(
                caseNs,
                @case.EmitName ?? @case.Name,
            ...
            if (union.TypeParams.Count > 0)
            {
                ...
                caseType.BaseType = baseType
                    .MakeGenericInstanceType(false, closedBaseArgs)
                    .ToTypeDefOrRef();
            }
            ...
            // Case constructor
            var caseCtorParams = @case.Fields.Select(f => MapToClr(f.Type, EmitContext.Empty, typeParamMap: typeParamMap)).ToArray();
            ...
            for (var i = 0; i < @case.Fields.Count; i++)
                caseCtor.ParameterDefinitions.Add(
                    new ParameterDefinition((ushort)(i + 1), Sanitize(@case.Fields[i].Name), 0)
                );
            ...
            if (union.TypeParams.Count > 0)
            {
                ...
                var closedBaseCtor = new MemberReference(
                    closedBaseSig.ToTypeDefOrRef(),
                    ".ctor",
                    MethodSignature.CreateInstance(_module.CorLibTypeFactory.Void)
                );
