...
{
    /// <summary>
    ///     How the first line of every generated C# file starts, unless the preamble is
    ///     suppressed. Lets a tool tell a file this compiler wrote apart from a hand-written
    ///     one sitting in the same directory — see
    ///     <see cref="CSharpProjectGenerator.WriteProjectDirectory" />'s pruning.
    /// </summary>
    internal const string GeneratedFileMarker = "// <auto-generated by ZScheme compiler";

    /// <summary>
    ///     The whole program as a single C# file: <see cref="EmitUnits" />'s header and
    ///     every unit body concatenated back together, byte for byte.
    /// </summary>
    public string Emit(IrNode node)
    {
        var units = EmitUnits(node);
        return units.Header + string.Concat(units.Units.Select(u => u.Body));
    }

    /// <summary>
    ///     Emits the program once and returns it partitioned into a shared file header and
    ///     one unit per emitted class, so a caller can write each unit as its own source
    ///     file. The split is a slice of a single emission pass rather than one pass per
    ///     unit: the emitter carries state across modules — the local-name counter behind
    ///     <c>__match{n}</c>/<c>__sym{n}</c>, the emitted-class table a later module's base
    ///     class resolves through, and the CLR type/assembly map — so separate emitters
    ///     would not reproduce this output.
    /// </summary>
    /// <remarks>
    ///     Single-use per instance. <c>_sb.Clear()</c> does not reset that cross-module
    ///     state, so a second call on the same emitter would not repeat the first's output.
    /// </remarks>
    public CSharpEmitUnits EmitUnits(IrNode node)
    {
        Log.Debug(
        ...
        if (!suppressVersionPreamble)
            EmitLine($"{GeneratedFileMarker} {CompilerInfo.VersionString}>");
        ...
        var units = new List<CSharpEmitUnit>(marks.Count);
        for (var i = 0; i < marks.Count; i++)
        {
            var end = i + 1 < marks.Count ? marks[i + 1].Start : all.Length;
            units.Add(new CSharpEmitUnit(marks[i].ModuleClassName, all[marks[i].Start..end]));
        }

        return new CSharpEmitUnits(all[..marks[0].Start], units);
    }
