...
public static class ZTypeSerializer
{
    public static JsonNode Serialize(ZType type)
    {
        return type switch
        {
            ...
            ZType.ZTypeVar tv => new JsonObject { ["kind"] = "typeVar", ["id"] = tv.Id },
    ...
    {
        var obj =
            node as JsonObject ?? throw new ArgumentException("Expected a JSON object for ZType");
        ...
        return kind switch
        {
            ...
            "nullable" => new ZType.ZNullableType(
                Deserialize(
                    obj["inner"]
                        ?? throw new ArgumentException(
                            "Missing 'inner' field in nullable ZType JSON"
                        )
                )
            ),
    ...
    {
        var primitiveKindStr =
            obj["primitiveKind"]?.GetValue<string>()
    ...
    {
        var paramsNode =
            obj["params"] as JsonArray
            ?? throw new ArgumentException("Missing or invalid 'params' field in fn ZType JSON");

        var returnNode =
            obj["return"] ?? throw new ArgumentException("Missing 'return' field in fn ZType JSON");

        var paramTypes = new List<ZType>(paramsNode.Count);
        foreach (var p in paramsNode)
            paramTypes.Add(
                Deserialize(p ?? throw new ArgumentException("Null element in 'params' array"))
            );
    ...
    private static ZType DeserializeNamedType(JsonObject obj)
    {
        ...
        var argsNode =
            ...
            ?? throw new ArgumentException(
                "Missing or invalid 'typeArgs' field in named ZType JSON"
            );

        var typeArgs = new List<ZType>(argsNode.Count);
        foreach (var arg in argsNode)
            typeArgs.Add(
                Deserialize(arg ?? throw new ArgumentException("Null element in 'typeArgs' array"))
            );
    ...
    {
        var varsNode =
            obj["boundVars"] as JsonArray
            ?? throw new ArgumentException(
                "Missing or invalid 'boundVars' field in forall ZType JSON"
            );

        var bodyNode =
            obj["body"] ?? throw new ArgumentException("Missing 'body' field in forall ZType JSON");
    ...
    private static ZType DeserializeConstrainedVar(JsonObject obj)
    {
        ...
        var kindsNode =
            obj["allowedKinds"] as JsonArray
            ?? throw new ArgumentException(
                "Missing or invalid 'allowedKinds' field in constrained ZType JSON"
            );
