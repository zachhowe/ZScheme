...
public sealed partial class Compilation(CompilerOptions? options = null)
{
    ...
    private static TypeAliasRegistry CreateDefaultRegistry()
    {
        ...
        registry.RegisterBuiltIn(new TypeAliasInfo(
            "Task", [], "System.Threading.Tasks.Task`1", "System.Threading.Tasks", TypeAliasKind.GenericClrType,
            default));
        registry.RegisterBuiltIn(new TypeAliasInfo(
            "System.Threading.Tasks.Task", [], "System.Threading.Tasks.Task`1", "System.Threading.Tasks",
            TypeAliasKind.GenericClrType, default));
        registry.RegisterBuiltIn(new TypeAliasInfo(
    ...
    public CompilationResult Compile(string source, string fileName = "input.zs")
    {
        ...
        var (_, compiledModules2, importErrors) =
            CompileResolveAndCompileImports(preImports, compiledModules, resolver);
        if (importErrors)
            return new CompilationResult.DependencyResolutionFailure(_diagnostics);
        ...
        sw.Restart();
        var (inferer, typeInferenceErrors) =
            CompileTypeInference(program!, compiledModules, _options.PrimaryModuleName, hasModuleDecl, sw);
    ...
    /// </summary>
    private (List<Token> Tokens, List<SExpr> SExprs, bool HasLexErrors) CompileLexAndParse(string source,
        string fileName, Stopwatch sw)
    {
        // Stage 1: Lex
    ...
    /// </summary>
    private (AstNode.Program Program, List<AstNode.Import> Imports, bool IsPreludeModule, HashSet<string>
        UserImportNames)
    ...
    private void CompilePreludeModules(
        ...
        HashSet<string> userImportNames)
    {
        if (disablePrelude || isPreludeModule) return;
    ...
    private (List<AstNode.Import> Imports, List<CompiledModule> CompiledModules, bool HasErrors)
        CompileResolveAndCompileImports(
            ...
            ModuleResolver resolver)
    {
        if (preImports.Count <= 0) return (preImports, compiledModules, false);
    ...
    /// </summary>
    private (List<SExpr> SExprs, bool HasErrors) CompileExpandMacros(List<SExpr> sexprs,
        List<CompiledModule> compiledModules, Stopwatch sw)
    {
            ...
            else
            {
                ...
                if (firstDefine is not null)
                {
                    _diagnostics.Error("Files with top-level definitions require a (module ...) declaration",
                        firstDefine.Span);
                    return (program, defaultClassName, false,
    ...
    private (TypeInferer Inferer, bool HasErrors)
        CompileTypeInference(AstNode.Program program, List<CompiledModule> compiledModules, string? primaryModuleName,
            bool hasModuleDecl, Stopwatch sw)
    {
        ...
        var inferer = new TypeInferer(_diagnostics, _options.AssemblySearchPaths, TypeAliases)
        {
            ...
            CurrentModuleName = primaryModuleName ?? (hasModuleDecl
                ? program.TopLevelForms
        ...
        bool suppressVersionPreamble)
    {
        ...
        if (_options.OutputMode == OutputMode.CSharp)
        {
            Log.Debug(
                ...
                _options.Namespace, className, clrNamespaces.Count, csImportedModules.Count,
        ...
        Log.Debug(
            ...
            _options.Namespace, className, clrNamespaces.Count, hoistedSourceImportedModules.Count,
