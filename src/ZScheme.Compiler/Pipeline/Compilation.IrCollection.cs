...
public sealed partial class Compilation
{
    ...
    private void CollectTypeAliasesFromAst(AstNode node)
    {
        switch (node)
        {
            case AstNode.Program p:
                foreach (var f in p.TopLevelForms) CollectTypeAliasesFromAst(f);
                break;
            case AstNode.ModuleDecl m:
                foreach (var f in m.Body) CollectTypeAliasesFromAst(f);
                break;
            case AstNode.TypeAliasDecl alias:
                var info = new TypeAliasInfo(
                    alias.AliasName, alias.TypeParams, alias.ClrTarget, alias.AssemblyHint,
    ...
    private void CollectTypeAliases(IrNode node)
    {
        switch (node)
        {
            case IrNode.Seq seq:
            {
                foreach (var child in seq.Nodes) CollectTypeAliases(child);
                break;
            }
            case IrNode.Let let:
