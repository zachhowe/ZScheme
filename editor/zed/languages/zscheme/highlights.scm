...
(list head: (symbol) @keyword
  (#match? @keyword "^(define-syntax|define-async|define-type-alias|define-record|define-struct|define-union|define-class|define-interface|define|record|struct|union|class|interface|letrec|let\\*|let|use\\*|use|if|lambda|match|with-handlers|with|set!|begin|new|raise|await|syntax-rules|typeof|delegate|values|quote)$"))
