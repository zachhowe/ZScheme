...
- **Generated `.cs` files are pruned before writing.** The csproj carries no `<Compile>`
  ...
  source file behind that still compiles. `WriteProjectDirectory` deletes only files whose
  first line carries the `// <auto-generated by ZScheme compiler` marker, so a hand-written
  source in the output directory survives.
