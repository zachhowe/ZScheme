...
- **Generated `.cs` files are pruned before writing.** The csproj carries no `<Compile>`
  items, so the SDK globs `**/*.cs`: a module renamed since the last run would leave a
  source file behind that still compiles. `WriteProjectDirectory` — which `zs compile
  --emit-project` shares — deletes only files whose first line carries the
