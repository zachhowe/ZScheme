...
- **Generated `.cs` files are pruned before writing**, so a renamed module's old file does
  ...
  so a hand-written source in the output directory survives. `bin/`, `obj/` and anything
  behind a symlink are left alone. Only `generate-project` prunes: its two project
