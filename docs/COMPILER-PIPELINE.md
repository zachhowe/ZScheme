...
- **The main project is split by slicing one emission, not by emitting each module
  separately.** `CSharpEmitter.EmitUnits` returns the shared file header plus one unit per
  class, all from a single pass, and `Emit` is that same result concatenated. The emitter
  carries state across modules — the counter behind `__match{n}` local names, the
  emitted-class table a later module's base class resolves through — so a per-module
  emitter would not reproduce the same code. `LibraryCompiler.RelativePathForModule` maps
  each unit to its file.
- **The csproj names its sources.** Every file written is an explicit `<Compile>` item and
  the SDK's default `**/*.cs` glob is switched off, so a stray `.cs` in the output directory
  — a module's file from before it was renamed, a hand-written source — is never compiled
  into a duplicate definition. Only the manifest-less `generate-project`, which writes a
  csproj for sources the user adds by hand, keeps the glob.
- **Generated `.cs` files are pruned before writing**, so a renamed module's old file does
  not linger in the tree as if it were part of the project. `WriteProjectDirectory` deletes
  only files whose first line carries the `// <auto-generated by ZScheme compiler` marker,
  so a hand-written source in the output directory survives. `bin/` and `obj/` are left
  alone, and a directory link is not descended into; a generated file that is itself a link
  is pruned (the link, not its target). Only `generate-project` prunes: its two project
  directories are the package's alone. `zs compile --emit-project` shares
