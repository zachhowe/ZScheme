...
- **Generated `.cs` files are pruned before writing**, so a renamed module's old file does
  not linger in the tree as if it were part of the project. `WriteProjectDirectory` — which
  `zs compile --emit-project` shares — deletes only files whose first line carries the
  `// <auto-generated by ZScheme compiler` marker, so a hand-written source in the output
  directory survives. `bin/`, `obj/` and anything behind a symlink are left alone.
