...
- **`generate-project` writes the main project as one `.cs` per module.** It already split the
  ...
  - Generated `.cs` files are now pruned before a project is written, by `generate-project` and
    `zs compile --emit-project` alike. With no `<Compile>` items the SDK globs `**/*.cs`, so a
    module renamed since the last run — or a per-module tree left where `--emit-project` now
    writes its single file — would leave behind a source that still compiles into a duplicate
    definition. Only files whose first line carries the `// <auto-generated by ZScheme compiler`
    marker are removed, so a hand-written source in the output directory survives; `bin/` and
    `obj/` are left alone, and a symlink or junction inside the output directory is not followed.
