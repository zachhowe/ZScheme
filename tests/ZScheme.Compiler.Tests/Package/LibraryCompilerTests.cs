...
public class LibraryCompilerTests
{
    ...
    [Fact]
    public void CompileToCSharp_ModuleWithDependency_EmitsBothClasses()
    {
        var dir = CreateTempDir();
        try
        {
    ...
    [Fact]
    public void CompileToCSharp_EmitsOneFilePerModule()
    {
        var dir = CreateTempDir();
        try
        {
            ...
            Assert.Equal(
                ["base-mod.cs", "derived.cs"],
                result.Files.Select(f => f.RelativePath).Order(StringComparer.Ordinal)
            );

            var baseFile = result.Files.Single(f => f.RelativePath == "base-mod.cs");
            var derivedFile = result.Files.Single(f => f.RelativePath == "derived.cs");
            Assert.Contains("BaseModModule", baseFile.Source);
            Assert.DoesNotContain("class DerivedModule", baseFile.Source);
            Assert.Contains("DerivedModule", derivedFile.Source);
            Assert.DoesNotContain("class BaseModModule", derivedFile.Source);

            // Each file stands alone: its own header, and no leading blank line left over
            // from the blank the single-file form uses to separate module classes.
            foreach (var file in result.Files)
            {
                Assert.Contains("namespace ", file.Source);
                Assert.StartsWith("// <auto-generated", file.Source);
            }

            RoslynCompileVerifier.AssertCompiles(
                [.. result.Files.Select(f => f.Source)],
                result.PrecompiledDependencyPaths
            );
        }
        finally
        {
    ...
    /// <summary>
    ///     A module's file mirrors where it sits in the source tree, with the package's own
    ///     import prefix stripped — the prefix is how the module is imported, not where it
    ///     lives. A dependency inlined from another package keeps its prefix as a folder.
    /// </summary>
    [Fact]
    public void CompileToCSharp_FilePathsMirrorTheSourceTree()
    {
        var dir = CreateTempDir();
        try
        {
            ...
            File.WriteAllText(
                Path.Combine(dir, "top.zs"),
                "(module mypkg/top)\n(export top-fn)\n(define (top-fn) : Int 1)"
            );
            File.WriteAllText(
                Path.Combine(dir, "inner", "nested.zs"),
                "(module mypkg/inner/nested)\n(export nested-fn)\n(define (nested-fn) : Int 2)"
            );
            ...
            var result = compiler.CompileToCSharp(
                dir,
                MakeManifest(importPrefix: "mypkg"),
                MakeOptions()
            );
    ...
    [Theory]
    // The package's own prefix is stripped; a dependency's is kept as a folder.
    [InlineData("mypkg/list", "mypkg", "list.cs")]
    [InlineData("mypkg/mutable/vector", "mypkg", "mutable/vector.cs")]
    [InlineData("stdlib/list", "mypkg", "stdlib/list.cs")]
    [InlineData("list", null, "list.cs")]
    // Escaping the project directory, or landing where the SDK excludes sources from the
    // compilation, falls back to the class name.
    ...
    )
    {
        ...
        var path = LibraryCompiler.RelativePathForModule(moduleName, className, importPrefix, []);

        Assert.Equal(expected, path);
    }

    [Fact]
    public void RelativePathForModule_DisambiguatesACollision()
    {
        var taken = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        Assert.Equal(
            "list.cs",
            LibraryCompiler.RelativePathForModule("mypkg/list", "AModule", "mypkg", taken)
        );
        // Same path, different module: the second one falls back to its class name rather
        // than overwriting the first one's file.
        Assert.Equal(
            "BModule.cs",
            LibraryCompiler.RelativePathForModule("other/list", "BModule", "other", taken)
        );
    }
