...
public class CSharpProjectGeneratorTests
{
    ...
    [Fact]
    public void WriteProjectDirectory_WithoutPruning_LeavesExistingFilesAlone()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"zs-test-{Guid.NewGuid():N}");
        try
        {
            ...
            File.WriteAllText(
                ...
                "// <auto-generated by ZScheme compiler 0.0.0>\n"
            );
            ...
            Assert.True(File.Exists(Path.Combine(tempDir, "stale.cs")));
        }
        finally
        {
            if (Directory.Exists(tempDir))
